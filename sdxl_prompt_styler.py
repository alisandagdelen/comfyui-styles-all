...
class Template:
    ...
    def replace_prompts_advanced(
        self, positive_prompt_g, positive_prompt_l, negative_prompt, negative_prompt_to
    ):
        ...
        text_l_positive = ", ".join(
            x for x in (template_prompt_l_template, positive_prompt_l) if x
        )
        text_positive = (
            f"{text_g_positive} . {text_l_positive}"
            if text_l_positive
            else text_g_positive
        )
        text_negative = ", ".join(
            x for x in (self.negative_prompt, negative_prompt) if x
        )
...
class SDXLPromptStylerAdvanced:
    ...
    def prompt_styler_advanced(
        ...
        **kwargs,
        ...
        text_positive_g_styled, text_positive_l_styled, text_negative_styled = (
            text_positive_g,
            text_positive_l,
            text_negative,
        )
...
NODES = {
    ...
    **{f"{name.title()} Styler": (name,) for name in styler_data.keys()},
    ...
    #'ComfyUI Styler': [x for x in styler_data.keys() if x not in ('artist', 'milehigh')],
...
for k, v in NODES.items():
    assert not isinstance(
        v, str
    ), f"Error: {k} has a string instead of a sequence for its menu list. Did you forget a comma?"
...
NODE_CLASS_MAPPINGS = {
    **{
        name.replace(" ", ""): type(
            name.replace(" ", ""), (SDXLPromptStyler,), {"menus": menus}
        )
        for name, menus in NODES.items()
    },
    **{
        name.replace(" ", "")
        + "Advanced": type(
            name.replace(" ", "") + "Advanced",
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    **{
        name.replace(" ", "") + "Advanced": name + " (Advanced)"
